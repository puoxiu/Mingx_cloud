...
void MyFileWg::addUploadFiles()
{
    ...
    for(int i = 0; i < list.size(); ++i)
    {
        ...
        //  -3: 打开文件失败
        //  -4: 获取布局失败
        int res = uploadList->appendUploatList(list.at(i));
        if(res == -1)
        {
...
void MyFileWg::uploadFile(UploadFileInfo *info)
{
    ...
    MyProgressBar *progress = info->progress;        //进度条控件
    ...
    */
    ...
    connect(reply, &QNetworkReply::uploadProgress, [=](qint64 bytesRead, qint64 totalBytes)
    {
        if(totalBytes != 0) //这个条件很重要
        {
            ...
            progress->setValueProgress(bytesRead/1024, totalBytes/1024); //设置进度条
        }
    });
    ...
    connect(reply, &QNetworkReply::finished, [=]()
    {
        if (reply->error() != QNetworkReply::NoError) //有错误
        {
            ...
            UploadTask *uploadList = UploadTask::getInstance();
            uploadList->delUploadTask(); //删除已经完成的上传任务
            return;
        }
...
void MyFileWg::downloadFilesAction()
{
    ...
    DownloadFileInfo *tmp = p->takeTask(); //取下载任务
    ...
    MyProgressBar *progress = tmp->progress;
    ...
    {
        progress->setValueProgress(bytesRead, totalBytes);//设置进度
    }
    );
}
